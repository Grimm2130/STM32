...
#define EXTI_RTSR_OFF					0x08U
#define EXTI_PR_OFF						0x14U
...
int main(void)
{
	...
	// Address of GPIO_A input reg
//	uint32_t volatile* const port_a_input_reg_ptr = (uint32_t *) (GPIO_A_BASE_ADDR + GPIOx_IDR_OFF);

	printf("Program Started!\n");

    /* Loop forever */
	for(;;);
//		if( ( (*port_a_input_reg_ptr) & (1 << 0) ) ){
...
void en_ints(){
	...
	*sys_exti_cnfg_ptr &= ~(0xF << ENTI0_SRC_SET);
	...
	// Unmask the EXTI interrupt port
	uint32_t volatile* const exti_imr_ptr = (uint32_t *) (EXTI_BASE_ADDR);
	*exti_imr_ptr |= (1 << 0);

	// Set to trigger on rising edge
	uint32_t volatile* const exti_rtsr_ptr = (uint32_t *) (EXTI_BASE_ADDR + EXTI_RTSR_OFF);
	*exti_rtsr_ptr |= (1 << 0);
...
void setup_button(){
	...
	*port_a_mode_ptr &= ~(0x3 << PRT_A_BIT_START_MODE_X_PUPDR);
...
void EXTI0_IRQHandler(){
	...
	uint32_t volatile* const exti_pr_ptr = (uint32_t *) (EXTI_BASE_ADDR + EXTI_PR_OFF);
	*exti_pr_ptr |= (0x1 << 0);
}
