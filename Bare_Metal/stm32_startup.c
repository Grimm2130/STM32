 ...
 * @author Chima Ochiagha (ochiaghachima@gmail.com)
 * @brief Self-Defined Start up file for SMT32 Board
 * @version 0.1
 * @date 2023-03-08
 *
...
/*------------------- Globals ------------------*/
// An array of interrupts (vector Table)
// Initialized in "startup_sec"
uint32_t vectors[] __attribute__((section("startup_sec"))) = {
    (uint32_t) STACK_START,                // MSP address
    (uint32_t) &Reset_Handler,     // Address of the reset handler function pointer
    (uint32_t) &NMI_Handler,       // Address of the NMI handler function pointer
    (uint32_t) &HardFault_Handler, // Address of the Hard fault handler function pointer
...
// Reset handler
void Reset_Handler(void){
    // Code to move .data from FLASH to SRAM
